...
func main() {
	...
	p := PersonSlice{NewPerson("Anna", "Man"), NewPerson("Qwerty", "Toy"), NewPerson("Biba", "Buba")}
	fmt.Println("Before:", p)
	sort.Sort(p)
	fmt.Println("After:", p)

	fmt.Println("IsPalindrome Redux")
	f1 := NewPerson("Mr.", "Bobobo")
	f2 := NewPerson("Mr.", "Bububu")
	them := PersonSlice{f1, f2, f1}
	fmt.Printf("IsPalindrome(%v) = %v\n", them, IsPalindrome(them))

	fmt.Println("Functional Programming")
	add := func(x, y int) int { return x + y }
	fmt.Println("Fold([]int{1, 2, 3, 4, 5}, 0, add) =", Fold([]int{1, 2, 3, 4, 5}, 0, add))
	mult := func(x, y int) int { return x * y }
	fmt.Println("Fold([]int{1, 2, 3, 4, 5}, 1, mult) =", Fold([]int{1, 2, 3, 4, 5}, 1, mult))
}
